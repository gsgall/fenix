...
registerMooseObject("FenixApp", CurrentDensityRayKernel);
registerMooseObject("FenixApp", ADCurrentDensityRayKernel);
...
CurrentDensityRayKernelTempl<is_ad>::validParams()
{
  ...
  params.addClassDescription("RayKernel for calculating the current source from particles while they are being traced.");
...
CurrentDensityRayKernelTempl<is_ad>::computeQpResidual()
{
  ...
  return -ray->data(_charge_index) * ray->data(_weight_index) * ray->direction()(_component) * _test[_i][_qp] / this->_dt;
}

template class CurrentDensityRayKernelTempl<false>;
